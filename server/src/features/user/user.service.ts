...
export class UserService {
  ...
  public async findUserByUsername(username: string, currentUserId?: string) {
    ...
    // If a currentUserId is provided, check the Follow table
    if (currentUserId) {
  ...
  /**
   * --- NEW: Follow a user ---
   * Creates a follow relationship. followerId follows followingId.
   */
  public async followUser(followerId: string, followingId: string) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      // Denormalization: We could increment follower/following counts here
      // on the User model if we added those fields to the schema.
      // For now, we just return the follow record.
  ...
  /**
   * --- NEW: Unfollow a user ---
   * Deletes a follow relationship. followerId unfollows followingId.
   */
  public async unfollowUser(followerId: string, followingId: string) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      // Denormalization: Decrement counts if they exist on the User model.
