...
export const generateAccessToken = (user: User): string => {
  // --- FIX: Conditionally construct the payload to satisfy 'exactOptionalPropertyTypes'
 ...
 * Throws an HttpError if validation fails.
 * @returns The decoded payload of the valid refresh token.
 */
export const verifyAndValidateRefreshToken = async (
  ...
  try {
    ...
    // 2. Check token against database store
    ...
    if (storedToken.userId !== decoded.id) {
      // This is a serious security issue, invalidate the token and throw.
